...
// Process the grid Tiles outside of the central Box.
void oskar_process_tiles_excluding_box_oversample(
	   const int num_w_planes, 
	   const int* support,
	   ...
	   const int tileHeight,
	   const Point numTiles,
	   const Point exclusionBoxTop,
	   const Point exclusionBoxBot,
	   const std::vector<int> &numPointsInTiles,
	   const std::vector<int> &offsetsPointsInTiles,
	   const std::vector<float> & bucket_uu,
	   const std::vector<float> & bucket_vv,
	   const std::vector<float> & bucket_ww,
	   const std::vector<float2> & bucket_vis,
	   const std::vector<float> & bucket_weight,
	   const std::vector<Tile> &workQueue,
	   double* norm, 
	   float* grid
	   )
{
...
#define NUM_POINTS_IN_TILES(uu, vv)  numPointsInTiles.at( (uu) + (vv)*numTiles.u)
#define OFFSETS_IN_TILES(uu, vv)     offsetsPointsInTiles.at( (uu) + (vv)*numTiles.u)
  ...
  for (int tile=0; tile<nTiles; tile++)
    {
      ...
      int pu = workQueue.at(tile).pu;
      int pv = workQueue.at(tile).pv;
	    
      if(exclusionBoxTop.u <= pu && pu <= exclusionBoxBot.u &&
	 exclusionBoxTop.v <= pv && pv <= exclusionBoxBot.v)
	continue;

#ifdef USEASSERTS
      assert(pu<numTiles.u);
      assert(pv<numTiles.v);
#endif
...
#ifdef USEASSERTS
      assert(workQueue.at(tile).vis == num_tile_vis);
#endif
	...
	{
	  // Convert UV coordinates to grid coordinates.
	  float pos_u    = -bucket_uu.at(off+i) * scale;
	  float pos_v    = bucket_vv.at(off+i) * scale;
	  float ww_i     = bucket_ww.at(off+i);
	  const float w  = bucket_weight.at(off+i);
	  float2 val     = bucket_vis.at(off+i);
	  val.x *= w;
	  val.y *= w;
	  ...
	  //          boxTop.u + pu*tileWidth, boxTop.u + (pu+1)*tileWidth - 1
	  // and the same for pv.  Our grid iteration range is
	  //       grid_u + k
	  // for -wsupport <= k <= wsupport and 
	  //       grid_v + j
	  // for -wsupport <= j <= wsupport
	  const int tile_u[] = {boxTop.u + pu*tileWidth, boxTop.u + (pu+1)*tileWidth-1};
	  const int tile_v[] = {boxTop.v + pv*tileHeight, boxTop.v + (pv+1)*tileHeight-1};
...
#ifdef USEASSERTS		 
		  assert(tile_u[0]<=grid_u+k && grid_u+k<=tile_u[1]);
		  assert(tile_v[0]<=grid_v+j && grid_v+j<=tile_v[1]);
#endif
...
// Process the central box of the grid, which is where a large proportion of the visibilities,
// and hence computational effort, will lie.
void oskar_process_tiles_central_box_oversample(
	 const int num_w_planes, 
	 const int* support,
	 const int oversample, 
	 const int* compact_wkernel_start,
	 ...
	 const int tileHeight,
	 const Point numTiles,
	 const Point tileStart,
	 const Point centralBoxSize,
	 const std::vector<int> &numPointsInTiles,
	 const std::vector<int> &offsetsPointsInTiles,
	 const std::vector<float> & bucket_uu,
	 const std::vector<float> & bucket_vv,
	 ...
	 )
{
  ...
  // How many Tiles does this central box consist of?
  const int nTiles = centralBoxSize.u * centralBoxSize.v;

  const int loc_grid_u =  centralBoxSize.u * tileWidth;
  const int grid_u_min = (boxTop.u+(tileStart.u*tileWidth));
  const int grid_v_min = (boxTop.v+(tileStart.v*tileHeight));
   
#define NUM_POINTS_IN_TILES(uu, vv)  numPointsInTiles.at( (uu) + (vv)*numTiles.u)
#define OFFSETS_IN_TILES(uu, vv)     offsetsPointsInTiles.at( (uu) + (vv)*numTiles.u)

  // An array storing the points in each Tile of the central box.
  std::vector<int> ptsPerTile;
  ptsPerTile.resize(nTiles);

  std::vector<int> cenTileOff;
  cenTileOff.resize(nTiles);
   
  // How many visibilities are there in the central box?
  int num_central_points=0;
  for (int tile=0; tile<nTiles; tile++)
    {
      int pv = tileStart.v + (tile/centralBoxSize.u);
      int pu = tileStart.u + (tile%centralBoxSize.u);
      int pts = NUM_POINTS_IN_TILES(pu,pv);
      num_central_points += pts;
      ptsPerTile.at(tile) = pts;
    }

  cenTileOff.at(0) = 0;
  for(int tile=1; tile<nTiles; tile++)
    {
      cenTileOff.at(tile) = cenTileOff.at(tile-1)+ptsPerTile.at(tile-1);
    }
  ...
  const int loc_grid_size = nTiles*tileWidth*tileHeight;
  
  auto start = std::chrono::high_resolution_clock::now();

#ifdef USEASSERTS
  const int loc_grid_v =  centralBoxSize.v * tileHeight;
  assert(nTiles*tileWidth*tileHeight == loc_grid_u * loc_grid_v);
#endif
...
#pragma omp single
    {
      // One thread allocates large shared array containing enough space for each thread to have its own local central box copy.
      local_grid = (float*)calloc(loc_grid_size*nthreads, 2*sizeof(float));
    }

    // First of all every thread updates its own copy of the central box with its share of
    // the visibilities that fall in that central box. This needs no atomics or synchronisation.
#pragma omp for reduction(+:local_norm) schedule(guided, 5)
    for (int pt=0; pt<num_central_points; pt++)
      {
	const int myTile = tileLookup(cenTileOff,pt);
	const int pv = tileStart.v + (myTile/centralBoxSize.u);
	const int pu = tileStart.u + (myTile%centralBoxSize.u);

	const int off = OFFSETS_IN_TILES(pu, pv);
	const int i = pt-cenTileOff.at(myTile);
	...
	assert(jstart <= jend);
        ...
        // u_fac and v_fac are used to allow cache-efficient indexing into the wkernel array.
	int u_fac = 0, v_fac = 0;
	if (off_u >= 0) u_fac = 1;
	if (off_u < 0) u_fac = -1;
	if (off_v >= 0) v_fac = 1;
...
#pragma omp simd
		...
		p = (2*ithread*loc_grid_size) + (2*(((grid_v-grid_v_min + j) * loc_grid_u) + grid_u-grid_u_min + k));

#ifdef USEASSERTS
		assert(p>=0);
		assert(p + 1 < 2*nthreads*loc_grid_size);
#endif
	     
		local_grid[p]     += (val.x * c.x - val.y * c.y);
		local_grid[p + 1] += (val.y * c.x + val.x * c.y);
    ...
    int loc_grid_base = 2*(grid_v_min*grid_size + grid_u_min);

    // Second each thread updates one element of the overall grid's central box by summing
    // the values from each thread's local copy of the box. Because multiple threads don't
    // update the same overall grid point, this also needs no atomics/synchronisation.
#pragma omp for schedule(guided,5)
    for (int j=0; j<loc_grid_size; j++)
      {
	// Which row of the local grid?
	int loc_grid_row_v = j / loc_grid_u;

	// Which col of the local grid?
	int loc_grid_col_u = j % loc_grid_u;

	// Which global grid cell are we updating?
	int glob_idx = loc_grid_base + 2*((loc_grid_row_v*grid_size) + loc_grid_col_u);
	  
	for (int k=0; k<nthreads; k++)
	  {
	    // Which local grid index are we reading from?
	    long int grd_idx = (2*(long)k*(long)loc_grid_size)+(2*(long)j);

	    grid[glob_idx] += local_grid[grd_idx];
	    grid[glob_idx+1] += local_grid[grd_idx+1];
	  }
      }
...
#pragma omp single
  ...
  *norm += local_norm;
  ...
  auto stop = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> diff = stop-start;
  printf("Central box processing time: %gms\n", diff.count()*1000);
	...
	)
{
  ...
  *norm = 0.0;
  ...
  // Compute the central box where most of the work will be.
  ...
  // Define the size and location of the central box.
  Point centralBoxStart(0,0);
  Point centralBoxSize(0,0);   
  {
    // The central box consists of 2W x 2W Tiles. W is a tunable parameter.
    const int W = 3;
    int pu_center = (grid_size/2 - boxTop.u)/tileWidth;
    int pv_center = (grid_size/2 - boxTop.v)/tileHeight;
    centralBoxStart.u = pu_center-W;
    centralBoxStart.v = pv_center-W;
    centralBoxSize.u = 2*W;
    centralBoxSize.v = 2*W;
  }
  Point centralBoxStop( centralBoxStart.u+centralBoxSize.u-1, centralBoxStart.v+centralBoxSize.v-1);
  ...
  if(numTiles.u > 0 && numTiles.v > 0) 
    {
      ...
      // Process the whole grid minus the central box.

      oskar_process_tiles_excluding_box_oversample(num_w_planes, support, oversample, compacted_wkernel_start.data(),
						   compacted_wkernels.data(), cell_size_rad, w_scale, grid_size,
						   boxTop, tileWidth, tileHeight, numTiles,
						   centralBoxStart, centralBoxStop,
						   numPointsInTiles, offsetsPointsInTiles, 
						   bucket_uu, bucket_vv, bucket_ww, bucket_vis, bucket_weight,
						   workQueue, norm, grid);

      // Process just the central box.
      oskar_process_tiles_central_box_oversample(num_w_planes, support, oversample, compacted_wkernel_start.data(),
						  compacted_wkernels.data(), cell_size_rad, w_scale, grid_size,
						  boxTop, tileWidth, tileHeight, numTiles,
						  centralBoxStart, centralBoxSize,
						  numPointsInTiles, offsetsPointsInTiles, 
						  bucket_uu, bucket_vv, bucket_ww, bucket_vis, bucket_weight,
						  norm, grid);
